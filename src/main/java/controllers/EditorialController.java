...
public class EditorialController extends BaseController<Editorial> {
    ...
    public LinkedList<Editorial> updateNodes(String query) throws SQLException {
        return bdConnection.secureConnection(conn -> {
            try {
                PreparedStatement pr = conn.prepareStatement("select * from editorial " + query);
  ...
  public  Optional<Editorial> getNode(int code) throws SQLException {
        return this.updateNodes(String.format("where codigoedi = %s", code)).stream().findFirst();
    }
